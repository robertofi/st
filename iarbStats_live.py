...
TOL_LAST_CHECK_WITH_PORTFOLIO = 120
...
def show_trades():
    ...
    dfstrategiesAll = db.select(**diTables['strategies']).set_index('id')
    dfLogTrades = db.df_from_sql(**diTables['logTrades'])
    ...
    if cfg_i_arb.MODE == 'paper':
        ...
        dfLogTrades.set_index('id',inplace=True)
    ...
    dfLogTrades=dfLogTrades.fillna(np.nan)
    st.dataframe(
          dfLogTrades.loc[dfLogTrades.index[::-1], cols].style.set_precision(2).highlight_min(
              subset=['resultPerc', 'gross', 'pnl', 'assets'], color='blue').highlight_max(
                ...
                axis=1).format({
                'resultPerc': lambda x: f'{x:,.3%}' if not pd.isna(x) else '',
            ...
            tOT.dataframe(dfOpenTrades.style.bar(subset='dist', vmin=0, vmax=-2.2,
                ...
                lambda x: highlight_vals(x), axis=1).set_properties(**{'height': '10'}).format(
                  dict(lastCheck=lambda x: f'{x:,.0f}', id=lambda x: f'{x:,.0f}' if x > 0 else '',
                       ...
                       assets=lambda x: f'{x:,.0f}' if x else '')).set_precision(2))
        else:
            tiOT.text('')
            tOT.text('')
...
if __name__ == '__main__':
    ...
    # fileParams = cfg_i_arb.PATH + 'paperParams.json'
    diPaper = read_dict(cfg_i_arb.FILE_PARMS, add='')
    mo = manageOrders(Path=cfg_i_arb.PATH)
    db = mo.db
    mm = mo.mm
